...
export const LoginAction = {
    ...
    async settingsForm(scope: ActionScope) {
        ...
        scope.utils.eachComponent(scope.form.components, (component: any) => {
     ...
     * @returns {*}
     */
    async checkAttempts(scope: ActionScope, error: any, user: User) {
    ...
    async executor(scope: ActionScope) {
        ...
        return async (req: SubmissionRequest, res: SubmissionResponse, next: NextFunction) => {
            ...
            bcrypt.compare(password, hash.hash, async (err: any, value) => {
                if (err) {
                    error(err);
                    return next(err);
                }
                ...
                const errorAttempts = await LoginAction.checkAttempts(scope, !value, user);
                if (errorAttempts) {
                    error(errorAttempts);
